...
{
    public class ContentsDeveloper : Developer<LowLevelCollection<Video>>
    {
        public override TopLevelCollection<LowLevelCollection<Video>> Create()
        {
            var store = new ConfigFileStore<List<LowLevelCollection<Video>>>(Settings.GetConfigFilePath("contents.config"));
