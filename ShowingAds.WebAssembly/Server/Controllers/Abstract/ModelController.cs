...
namespace ShowingAds.WebAssembly.Server.Controllers.Abstract
{
        ...
        where TModelManager : IModelManager<TKey, TModel>
    {
        ...
        public async Task<ActionResult> GetPermittedModels()
        {
            ...
            var (isExists, user) = await manager.TryGet(x => x.Name == HttpContext.User.Identity.Name);
            if (isExists == false)
                return StatusCode(StatusCodes.Status401Unauthorized);
            var users = await manager.GetEmployeeUsers(user.Id).ToListAsync();
            var models = await _manager.GetPermittedModels(users);
        ...
        public async Task<ActionResult> AddOrUpdateModel([FromBody] TModel model)
        {
            ...
            var isSuccess = await _manager.TryAddOrUpdate(model);
            if (isSuccess)
        ...
        public async Task<ActionResult> DeleteModel([FromBody] JObject obj)
        {
            ...
            var (isSuccess, _) = await _manager.TryDelete(id);
            if (isSuccess)
                return StatusCode(StatusCodes.Status200OK);
            return StatusCode(StatusCodes.Status400BadRequest);
        }
