    ...
    <div class="card p-2 mb-0 mt-1">
        <div class="align-items-center pt-2 pr-2 pl-2">
            <button v-if="deleteCallback" type="button" class="close float-right">
                <b-icon-x variant="danger" @click="deleteCallback(client)" />
            </button>
            <button v-if="editCallback" type="button" class="close mr-3 float-right">
                <b-icon-sliders @click="editCallback(client)" />
            </button>
            <button :style="`background-color: rgb(${red}, ${green}, ${blue})`" class="card-text float-left btn" type="button" @click="clickCallback(client)">
    ...
    export default {
        ...
        computed: {
            user: function() {
                ...
                if (client == null)
                    return null
                return this.$store.state.manager.users ? this.$store.state.manager.users.find(x => x.id == client.account) : null
            },
            red: function() {
                let user = this.user
