...
{
    public abstract class NotifyModelManager<TKey, TModel, TManager> : MongoCachedModelManager<TKey, TModel, TManager>
        where TKey : struct
        where TModel : class, IModel<TKey>, ICloneable
        where TManager : Singleton<TManager>
    {
        public NotifyModelManager(IDataProvider<TKey, TModel> provider) : base(provider, Settings.DeviceCollection) { }
        ...
        public async new Task<bool> TryAddOrUpdateAsync(TKey key, TModel newValue)
        {
            ...
            if (isAdded)
                await NotifySubscribersAsync(Operation.CreateOrUpdate, key, newValue);
            return isAdded;
        }

        public async new Task<bool> TryAddOrUpdateWithoutProviderAsync(TKey key, TModel newValue)
        {
            var isAdded = await base.TryAddOrUpdateWithoutProviderAsync(key, newValue);
            if (isAdded)
                await NotifySubscribersAsync(Operation.CreateOrUpdate, key, newValue);
            return isAdded;
        }
        ...
        public async new Task<bool> TryDeleteAsync(TKey key, TModel model)
        {
            ...
            if (isDeleted)
                await NotifySubscribersAsync(Operation.Delete, key, model);
            return isDeleted;
        }

        public abstract Task<IEnumerable<Guid>> GetSubscribersAsync(TKey key, TModel model);

        protected async Task NotifySubscribersAsync(Operation operation, TKey key, TModel model)
        {
            var subscribers = await GetSubscribersAsync(key, model);
